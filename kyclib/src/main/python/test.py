...
def faceComparison(imgReal, imgDetect):
    ...
    if encodeReal == errorMSG:
        ...
        encodeTest = openVideo(imgDetect, encodeReal)
        if encodeTest == errorMSG:
            return errorMSG+" in Face image"
        else:
            return str(encodeTest)
...
def checkImage(image):
    ...
    encodeReal = face_recognition.face_encodings(img, faceLoc)
    if (len(encodeReal) > 0):
        return encodeReal[0]
    else:
        return "error"
...
def openVideo(videoName, encodeImage):
    ...
    while(vid_capture.isOpened()):
        ...
        encodeList.append(encodeCurFrame)
